...
# if more than one close they are awarded accordingly

class Scenario(BaseScenario):
    def make_world(self, args=None, seed=0):
        world = World()
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.view_radius = 0.5 #getattr(args, "agent_view_radius", -1)
            agent.show_view = True if agent.adversary else False
            if not agent.adversary:
                agent.caught = False
                agent.catchers = []
                agent.catchers.append([])
        ...
        world.walls = [
            # Wall(orient='H', axis_pos=-0.68, endpoints=(-0.4, 0.4), width=0.65),

            # Wall(orient='H', axis_pos=0.68, endpoints=(-0.4, 0.4), width=0.65),
            Wall(orient='H', axis_pos=10.95, endpoints=(-1, 1), width=20),
            Wall(orient='H', axis_pos=-10.95, endpoints=(-1, 1), width=20),
            # Wall(orient='V', axis_pos=0.0, endpoints=(-1.0, 1.75)),
            # Wall(orient='H', axis_pos=0.50, endpoints=(-1.0, -0.98), width=3),
            Wall(orient='V', axis_pos=-10.95, endpoints=(-1, 1), width=20),
            Wall(orient='V', axis_pos=10.95, endpoints=(-1.0, 1.75), width=20)
        ]
    ...
    def prey_policy(self, agent, world):
        if agent.caught:
            agent.state.p_pos = [random.uniform(-.8, +.8) for _ in range(world.dim_p)]
            agent.catchers.append([])
            agent.caught = False
        # return np.array([0, 0])
        action = None
        ...
        if self.score_function == "sum":
            for i in range(n_iter):
                ...
                scores[np.where((abs(proj_pos) > .9).any(axis=1))[0]] = -9999999
        ...
        if scores[best_idx] < 0:#  or self.bound(agent.state.p_pos+chosen_action):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            agent.color = np.array([0.35, 0.85, 0.35]) if not agent.adversary else np.array([0.95 - (i * 0.3), 0.05 + (i * 0.3), 0.05 + (i * 0.3)])
            if not agent.adversary:
                agent.caught = False
                agent.catchers = []
                agent.catchers.append([])
        ...
        for agent in world.agents:
            if agent.adversary:
                ...
                agent.state.p_pos = np.random.uniform(-.8, +.8, world.dim_p)
    ...
    def is_close(self, agent1, agent2):
        delta_pos = agent1.state.p_pos - agent2.state.p_pos
        dist = np.sqrt(np.sum(np.square(delta_pos)))

        if (agent1.view_radius >= 0) and dist <= agent1.view_radius:
            return True
        return False
    ...
    def adversary_reward(self, agent, world):
        ...
        dist2 = []
        for a in agents:
            dist2.append(np.sqrt(np.sum(np.square(a.state.p_pos - agent.state.p_pos))))

        # rew -= np.exp(-(dist2 ** 2) / 0.1)
        rew -= 0.1 * min(dist2)

        # this part diff from coop
        if agent.collide:
            for ag in agents:
                is_col = False
                predators = []
                for adv in adversaries:
                    if self.is_collision(adv, ag):
                        is_col = True
                        predators.append(adv)
                    elif self.is_close(adv, ag):
                        predators.append(adv)
                if is_col and agent in predators:
                    rew += 10
                    # rew += np.power(10, len(predators) - 1)
                    # if len(predators) == 1:
                    #     rew += 1
                    # else:
                    #     rew += 10
                    ag.caught = True
                    ag.catchers[-1].append(agent.name)
        return rew
