...
class World(object):
    ...
    def integrate_state(self, p_force):
        for i, entity in enumerate(self.entities):
            ...
            #
            #         wf = self.get_wall_collision_force(entity, wall)
            #         if wf is not None and np.sum(np.abs(wf)) > 2:
            #             for jj in range(2):
            #                 if wf[jj] > 0 and entity.state.p_vel[jj] < 0:
            #                     entity.state.p_vel[jj] = 0
            #                 elif wf[jj] < 0 and entity.state.p_vel[jj] > 0:
            #                     entity.state.p_vel[jj] = 0
    ...
    def get_collision_force(self, entity_a, entity_b):
        ...
        dist = np.sqrt(np.sum(np.square(delta_pos / self.scale))) # because of scale thing we are multiplying with this
    ...
    def get_wall_collision_force(self, entity, wall):
        ...
        if (ent_pos[prll_dim] < wall.endpoints[0] - entity.size or
            ...
            if not (ent_pos[perp_dim] < wall.axis_pos + 0.5 * wall.width + 1 * entity.size
                    and ent_pos[perp_dim] > wall.axis_pos - 0.5 * wall.width - 1 * entity.size):
                return None
            # part of entity is beyond wall
            if ent_pos[prll_dim] < wall.endpoints[0]:
                # ent_pos[perp_dim] < wall.axis_pos + 0.5 * wall.width and ent_pos[perp_dim] > wall.axis_pos - 0.5 * wall.width

                dist_min = entity.size
                # negative side, assigning angle accordingly
                theta = np.pi / 2 * - 1
                # getting the closest one's distance
                delta_pos = ent_pos[prll_dim] - wall.endpoints[0]
                ...
                theta = np.pi / 2
                # getting the closest one's distance
                delta_pos = ent_pos[prll_dim] - wall.endpoints[1]
                pass
        ...
        force = np.zeros(2)
        perp_f = np.cos(theta) * force_mag # if abs(np.cos(theta) * force_mag) >= 0.1 else 0
        prll_f = np.sin(theta) * np.abs(force_mag) #if abs(np.sin(theta) * np.abs(force_mag)) >= 0.1 else 0
        force[perp_dim] = perp_f
        force[prll_dim] = prll_f
        return force