...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 ...
                 discrete_action_input=False, width=1900, height=900):

        self.width = width
        self.height = height
        self.scale = (width/width, width/height)
        world.scale = self.scale
        self.world = world
    ...
    def render(self, mode='human'):
        ...
        if self.render_geoms is None:
            ...
            for entity in self.world.entities:
                if hasattr(entity, 'show_view') and entity.show_view:
                    geom = rendering.make_double_circle(entity.size, entity.view_radius, filled=False)
                else:
                    geom = rendering.make_circle(entity.size)
                xform = rendering.Transform(scale=self.scale)
                if isinstance(entity, multiagent.core.Agent):
