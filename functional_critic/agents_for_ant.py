...
class BaseAgent(nn.Module):
    ...
    def __init__(self, params, env, device, seed, centralized=False):
        ...
        if not centralized:
            # sisl
            if params['is_gym_env'] and env.discrete_action_space:
                self.is_env_discrete = False
                self.action_space = gym.spaces.Box(low=0.0, high=1.0, shape=(env.action_space[0].n,), dtype=np.float32)
                self.state_space = self.env.observation_space[0]

            elif params['is_gym_env']:
                self.is_env_discrete = False
                self.action_space = self.env.action_space[0]
                self.state_space = self.env.observation_space[0]
            # sisl
            elif params['is_sisl_env'] or params['is_mamujoco_env']:
                self.is_env_discrete = False
                self.action_space = list(self.env.action_spaces.values())[0]
                self.state_space = list(self.env.observation_spaces.values())[0]

            else:
                if type(self.env.action_space) == gym.spaces.Discrete:
                    self.is_env_discrete = True
                    # pd for each action
                    self.action_space = gym.spaces.Box(low=0.0, high=1, shape=(self.env.action_space.n,), dtype=np.float32)
            ...
            if params['is_gym_env'] and env.discrete_action_space:
                ...
                self.action_space = gym.spaces.Box(low=-1.0, high=1.0,
                                                   shape=(self.env.action_space[0]._shape[0] * self.env.n,),
                                                   dtype=np.float32)
                self.state_space = gym.spaces.Box(low=-np.inf, high=np.inf,
                                                  shape=(self.env.observation_space[0]._shape[0] * self.env.n,),
                                                  dtype=np.float32)

            elif params['is_sisl_env'] or params['is_mamujoco_env']:
                self.is_env_discrete = False
                # should get the boundaries from real env, rather than hardcoding
                self.action_space = gym.spaces.Box(low=-1.0, high=1.0,
                                                   shape=(list(self.env.action_spaces.values())[0]._shape[0] *
                                                          len(self.env.possible_agents),),
                                                   dtype=np.float32)
                self.state_space = gym.spaces.Box(low=-np.inf, high=np.inf,
                                                  shape=(list(self.env.observation_spaces.values())[0]._shape[0] *
                                                         len(self.env.possible_agents),),
                                                  dtype=np.float32)
            else:
                return 'not impl'
    ...
    def compute_Q_star(self, s, return_best_actions=False, quantile_sampling=False, step=-1, do_minq=False,
                       action_to_sample=None, actions=None):
        if action_to_sample is None:
            action_to_sample = self.action_to_sample
    ...
    def enact_policy(self, s, episode, step, train_or_test, policy_type="e_greedy"):
        ...
        policy_types = {
            ...
            'e_greedy_gaussian': self.e_greedy_gaussian,
        }
    ...
    def e_greedy_policy(self, s, episode, step, train_or_test):
        ...
        # epsilon = max(0.01, 1 - (1 - 0.01) * (episode / (0.8 * 10000)))
    ...
    def e_greedy_gaussian(self, s, episode, step, train_or_test):
        """
        Given state s, at episode, take random action with p=eps if training Note - epsilon is determined by episode
        :param s:
        :param episode:
