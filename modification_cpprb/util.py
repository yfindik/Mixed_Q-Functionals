...
from gymnasium.spaces import MultiDiscrete as MD
from gymnasium.spaces import MultiBinary as MB


def from_space(space, int_type: DTypeLike, float_type: DTypeLike) -> DictType:
    if isinstance(space, Discrete) or isinstance(space, discrete.Discrete):
        return {"dtype": int_type, "shape": 1}
    elif isinstance(space, MultiDiscrete) or isinstance(space, MD):
        return {"dtype": int_type, "shape": space.nvec.shape}
    elif isinstance(space, Box) or isinstance(space, box.Box):
        return {"dtype": float_type, "shape": space.shape}
    elif isinstance(space, MultiBinary) or isinstance(space, MB):
...
def create_env_dict(
    ...
    try:
        observation_space = env.observation_space if isinstance(env.observation_space, Box) else env.observation_space[
            0]
    except Exception as e:

        _t = lambda: None
        _t.observation_space = list(env.observation_spaces.values())
        _t.action_space = list(env.action_spaces.values())
        env = _t
        observation_space = env.observation_space if isinstance(env.observation_space, Box) else env.observation_space[
            0]
    ...
    if isinstance(env.action_space, list):
        _temp = _temp[0]
    if isinstance(_temp, Box) or isinstance(_temp, box.Box):
        action_space = _temp
    elif isinstance(_temp, Discrete) or isinstance(_temp, discrete.Discrete):
        # action_space = Box(low=0.0, high=1, shape=(_temp.n,), dtype=np.float32)
        action_space = _temp
    else:
        action_space = _temp[0]

    if isinstance(observation_space, Tuple) or isinstance(observation_space, tuple.Tuple):
    ...
    elif isinstance(observation_space, Dict) or isinstance(observation_space, dict.Dict):
    ...
    if isinstance(action_space, Tuple) or isinstance(action_space, tuple.Tuple):
        for i, s in enumerate(action_space.spaces):
            env_dict[f"act{i}"] = from_space(s, int_type, float_type)
    elif isinstance(action_space, Dict) or isinstance(action_space, dict.Dict):
